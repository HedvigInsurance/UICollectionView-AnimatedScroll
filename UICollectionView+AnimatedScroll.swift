...
//  UICollectionView+AnimatedScroll
//
...
public extension CAMediaTimingFunction {
    ...
    func valueAtTime(x: Double) -> Double {
        ...
        let y = cubicFunctionValue(a: Double(-cp[0].y + 3 * cp[1].y - 3 * cp[2].y + cp[3].y), Double(3 * cp[0].y - 6 * cp[1].y + 3 * cp[2].y), Double(-3 * cp[0].y + 3 * cp[1].y), Double(cp[0].y), t)
